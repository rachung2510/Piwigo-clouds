...
{footer_script require='jquery'}{literal}
    ...
    $(".pwg-icon-menu").click( function(e) {
        e.preventDefault();
        if ($(".categoryActions").is(":visible")) {
            $(".categoryActions").hide();
            $(".pwg-icon-menu").removeClass("active");
        } else {
            $(".categoryActions").css("display", "flex");
            $(".pwg-icon-menu").removeClass("active").addClass("active");
            hideAllDropdowns();
        }
    });
    ...
    $(window).resize ( function() {
        if ($(window).width() <= 480 ) {
            ...
            $(".pwg-icon-menu").removeClass("active"); // reset
        } else {
            $(".categoryActions").css("display", "flex"); // redisplay actions
        }
    });

    /**
     * Toggle active state of given element. Active element has grey icons and text.
     * Only runs in mobile state
     * @param elem Element to toggle active class
     */
    ...
    /**
     * Hides all active dropdowns and removes active classes.
     * @param exclude ID of element to exclude
     */
    function hideAllDropdowns(exclude=null) {
        $(".switchBox").each( function() {
            if (!$(this).is(exclude))
                $(this).hide(); // hide all inner dropdowns
        });
        $("#sortOrderLink").removeClass("active");
        $("#derivativeSwitchLink").removeClass("active");
    }
